  ...
  text.split(/[\r\n]+/).forEach((line) => {
    ...
    if (pathParams) {
      pathParams.forEach((paramMatch) => {
        const propsString = `{${_.trim(paramMatch, ':/')}}`;
        const paramDef = _.pick(fetchProps(propsString, { noSchema: true })[0], ['name', 'type']);
    ...
    if (queryParams) {
      queryParams.forEach((param) => {
        const paramDef = fetchProps(`{${param}}`, { noSchema: true })[0];
