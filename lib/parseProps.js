...
function parseSingleProp(prop, options = {}) {
  ...
  return parsedToDef(parseType(propPair[1] || propPair[0]), propPair, isOptional, options);
}

function parsedToDef({
  isRef,
  isEnum,
  isNestedObject,
  isCollection,
  ...
  type: extractedType,
  format,
}, propPair, isOptional, options) {
  if (propPair[1] && !isValid && !isNestedCollection) {
    throw new Error(`Definition \`${propPair[0]}\` is invalid`);
  }
  ...
  if (isCollection) {
    if (isNestedCollection) {
      // remove next top level array
      const unnested = propPair[1].replace(/\[\]$/, '');
      // recurse w/ unnested
      const parse = parsedToDef(
        parseType(unnested),
        ['', unnested],
        false,
        options,
...
function parseType(typeDef) {
  ...
  const isNestedCollection = _.endsWith(typeDef, '[][]');
